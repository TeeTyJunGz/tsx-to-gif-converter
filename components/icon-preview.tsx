 ...
 */
export function IconPreview({ viewBox, elements, groupAnim, config, label }: IconPreviewProps) {
  ...
  useEffect(() => {
    ...
    const loop = computeLoopDuration(config, elements, groupAnim)
    const start = performance.now()
    let raf = 0

    const tick = (now: number) => {
      const progress = config.animation === "none" ? 0 : ((now - start) / 1000 / loop) % 1
      node.innerHTML = renderSvgString({ viewBox, elements, config, size: config.size, progress, groupAnim })
      raf = requestAnimationFrame(tick)
    }
    raf = requestAnimationFrame(tick)
    return () => cancelAnimationFrame(raf)
  }, [viewBox, elements, groupAnim, config])
